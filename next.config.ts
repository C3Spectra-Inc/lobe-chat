...
const nextConfig: NextConfig = {
  ...
  async headers() {
    ...
    const shouldApplyFrameHeaders = shouldUseCSP || !!frameAncestors;

    if (shouldApplyFrameHeaders) {
      const normalizedFrameAncestors = frameAncestors?.trim();

      if (normalizedFrameAncestors && normalizedFrameAncestors !== "'none'") {
        securityHeaders.push({
          key: 'Content-Security-Policy',
          value: `frame-ancestors ${normalizedFrameAncestors};`,
        });
      } else {
        securityHeaders.push(
          {
